...
import { getEmployeesHistoricalBalances, updateEmployeeYearlyBalance } from '../../actions/employee';
...
export default function BalancesManagement() {
  ...
  async function loadEmployeesBalance() {
    const result = await getEmployeesHistoricalBalances();
    if (result.success) {
  ...
  const handleUpdateDays = async (id, yearField, yearF, operation) => {
    const currentDays = yearF
    const newDays = operation === 'increment' ? currentDays + 1 : Math.max(0, currentDays - 1);
    console.log(id, yearField, newDays)
    const req = await updateEmployeeYearlyBalance(id, yearField, newDays)
    if(req.success){
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-[11px] font-black uppercase tracking-wider text-slate-400">
                ...
                <th className="p-4 text-center bg-indigo-50/40 text-indigo-700 font-black">2026</th>
                <th className="p-4 text-center text-slate-500 font-bold">2025</th>
                <th className="p-4 text-center text-slate-500 font-bold">2024</th>
                <th className="p-4 text-center text-slate-500 font-bold">2023</th>
              </tr>
            </thead>
              ...
              {employees.length === 0 ? (
                ...
                filteredEmployees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-slate-50/40 transition-colors duration-150">
                    ...
                    {['y2026', 'y2025', 'y2024', 'y2023'].map((year) => (
                      <td key={year} className={`p-4 text-center ${year === 'y2026' ? 'bg-indigo-50/10' : ''}`}>
                        <div className="flex items-center justify-center gap-2.5">
                          <button 
                            onClick={() => handleUpdateDays(emp.id, year, emp[year], 'decrement')}
                          ...
                          <span className="font-mono font-black text-slate-800 w-7 inline-block">{emp[year]}</span>
                          
                          <button 
                            onClick={() => handleUpdateDays(emp.id, year, emp[year], 'increment')}
